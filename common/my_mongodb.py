...
def init(mongodb_config):
    try:
        ...
        for db_name in mongodb_config:
            ...
            my_dbs[db_name] = MotorClient(db["HOST"], db["PORT"])[ db["DATABASE_NAME"] ]

            if db.get("USERNAME") and db.get("PASSWORD"):
                authenticate_list.append(my_dbs[db_name].authenticate(db["USERNAME"], db["PASSWORD"] ))

            setattr(my_dbs[db_name], "get_next_sequence", lambda sequence_name, db_name=db_name: get_next_sequence(my_dbs[db_name], sequence_name))
        ...
        raise tornado.gen.Return(my_dbs)
