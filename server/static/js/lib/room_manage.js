...
window.room_manage = {
    ...
    bind_event: function() {
        ...
        $('#btn_search').on('click', function(event){
            self.init_room_manage_list(self.setting.group_id, 1);
            return false;
        });
        ...
        $('#knowledgeScriptModal').on('hide.bs.modal', function (event) {
            ...
            $('#knowledgeScriptModal').attr('room_manage_id', null);
            common.clear_dynamic_div();
            $('.btn-outline-info[tag="record_input"]').hide();
        });
        $('#room_manage_table>tbody').on('click', 'a[tag="delete"]', function(event){
            var id = $(this).closest("tr").attr("dialogue_id");
            self.room_manage_delete(id);
            return false;
        });
        $('#room_manage_table>tbody').on('click', 'a[tag="edit"]', function(event){
            var id = $(this).closest("tr").attr("dialogue_id");
            self.build_modal(id);
            return false;
        });
        ...
        $('#btn_upsert_room_manage').on('click', function(){
            self.upsert_room_manage();
            return false;
        });
            ...
            success: function(json) {
                if (json.code !== 200) {
                    ...
                    $.each(json.data, function(i, item){
                        li_html.push([
                            '<li class="nav-item" group_id="' + item.roomid + '">',
    ...
    init_contact_detail: function() {
        ...
        var data = {
            'roomid': self.setting.group_id,
        }
        $.ajax({
            type: 'POST',
            url: '/api/room_message_list_query',
            data: JSON.stringify(data),
            contentType: 'application/json; charset=utf-8',
            dataType: 'json',
            success: function(json) {
